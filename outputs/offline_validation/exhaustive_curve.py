...
import sys
started=time.time();results=json.loads(Path(str(Path(__file__).resolve().parent / "results") + "/curve_execution_progress.json").read_text()) if "--resume" in sys.argv else []
for bank in range(len(results),7):
 ...
 Path(str(Path(__file__).resolve().parent / 'results') + '/curve_execution_progress.json').write_text(json.dumps(results,indent=2))
Path(str(Path(__file__).resolve().parent / 'results') + '/odyssey_curve_execution_verification.json').write_text(json.dumps({'rom_sha256':hashlib.sha256(ROM).hexdigest(),'method':'Ghidra SH-2A SLEIGH p-code execution via a local interpreter, compared against independently evaluated split fixed-point interpolation at every signed 16-bit input. ROM is immutable. No timing, interrupts, peripherals or physical plant modeled.','resumed_pass_elapsed_seconds':time.time()-started,'banks':results},indent=2)+'\n')
